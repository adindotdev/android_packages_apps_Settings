...
public abstract class CustomSettingsPreferenceFragment extends SettingsPreferenceFragment {
    ...
    protected static final int SYSTEM_SWITCH = 0;
    protected static final int SECURE_SWITCH = 1;
    protected static final int GLOBAL_SWITCH = 2;
    ...
    protected boolean updateCustomPreference(Preference preference, boolean clicked) {
        if (mCustomPreferences.containsKey(preference)) {
            ...
            if (preferenceType == SYSTEM_SWITCH || preferenceType == SECURE_SWITCH
                    || preferenceType == GLOBAL_SWITCH) {
                updateSwitchPreference(preference, preferenceType, clicked);
                return true;
            }
        }
        return false;
    ...
    private void updateSwitchPreference(Preference preference, int switchType, boolean clicked) {
        SwitchPreference switchPreference = (SwitchPreference) preference;
        int defaultValue = mCustomPreferences.get(preference).get(1).intValue();
        boolean isEnable = false;
        if (switchType == SYSTEM_SWITCH) {
            if (clicked) {
        ...
        } else if (switchType == SECURE_SWITCH) {
            if (clicked) {
            ...
            isEnable = (Settings.System.getInt(
                    getActivity().getContentResolver(), switchPreference.getKey(), defaultValue) == 1);
        } else if (switchType == GLOBAL_SWITCH) {
            if (clicked) {
